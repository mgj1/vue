...
export default {
  ...
  watch: {
    totalCount: function () {
      // 如果总数量为0, 直接不显示
      if (this.totalCount === 0||!this.userInfo.name) {
        this.isShow = false
        // return false
      }
    },
  ...
  computed: {
    ...
    payClass () {
      ...
      if(totalPrice<minPrice||!this.userInfo.name){
        return 'not-enough'
      }
      return 'enough'
    ...
    payText () {
      const {totalPrice} = this
      const {minPrice} = this.info
      if (totalPrice === 0||!this.userInfo.name) {
        return `￥${minPrice}元起送`
  ...
  methods: {
    ...
    pay(){
        if( this.totalPrice<this.minPrice||!this.userInfo.name){
          return;
        }
          //alert(this.food.name)
